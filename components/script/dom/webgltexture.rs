...
use canvas_traits::webgl::{webgl_channel, TexDataType, TexFormat, TexParameter, TexParameterInt, TexParameterBool, WebGLResult, WebGLTextureId};
...
impl WebGLTexture {
    ...
    pub fn maybe_get_tex_parameter(&self, param: TexParameter) -> Option<TexParameterValue> {
        match param {
            TexParameter::Int(TexParameterInt::TextureImmutableLevels) => Some(TexParameterValue::Int(self.immutable_levels.get().unwrap_or(0) as i32)),
            TexParameter::Bool(TexParameterBool::TextureImmutableFormat) => Some(TexParameterValue::Bool(self.is_immutable())),
            _ => None,
        }
    }
