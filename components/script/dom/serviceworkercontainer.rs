...
use script_traits::{Job, JobError, JobResult, JobType, ScriptMsg};
...
impl ServiceWorkerContainerMethods for ServiceWorkerContainer {
    ...
    fn Register(
                ...
                match result {
                    ...
                    JobResult::RejectPromise(error) => {
                        let promise = self
                            .trusted_promise
                            .take()
                            .expect("No promise to resolve for SW job.");
                        let _ = self.task_source.queue_with_canceller(
                            task!(reject_promise_with_security_error: move || {
                                ...
                                match error {
                                    JobError::TypeError => {
                                        promise.reject_error(Error::Type("".to_string()));
                                    },
                    ...
                    JobResult::ResolvePromise(job) => {
                        ...
                        let _ = self.task_source.queue_with_canceller(
                            task!(resolve_promise: move || {
                                ...
                                let registration =
                                    global.get_serviceworker_registration(&job.script_url, &job.scope_url);
        ...
        let mut handler = JobResultHandler {
